...
def build( modules, module_path, datadirs=None ):
	...
	if modules['nim']:
		...
		nimbin = os.path.expanduser('~/nim-0.13.0/bin/nim')
		niminclude = os.path.expanduser('~/nim-0.13.0/lib')
	...
	if modules['rusthon']:
		...
		for mod in mods_sorted_by_index:
			...
			compile_mode = 'binary'  ## c++ backend: binary, staticlib, dynamiclib
			...
			if backend == 'verilog':
				...
				if mod['tag'] and (mod['tag'].endswith('.h') or mod['tag'].endswith('.hpp') or mod['tag'].endswith('.cpp') or mod['tag'].endswith('.cc')):
					...
					output['datafiles'][ mod['tag'] ] = pak['main']  ## save output source code to tar
	...
	if cpp_merge:
		nuitka_funcs = []
		for mod in cpp_merge:
			merge = [mod['code']]

			if java2rusthon:  ## TODO fix me
				merge.extend(java2rusthon)
				java2rusthon = None
			if nim_wrappers:  ## TODO fix me
				## inserts generated rusthon nim wrappers into script before translation ##
				nim_wrappers.insert(0,'# nim wrappers generated by pythia #')
				nim_wrappers.insert(1, 'with extern(abi="C"):')
				merge.insert(0, '\n'.join(nim_wrappers))
				nim_wrappers = None
			if nuitka:  ## TODO fix me
			...
			#script = '\n'.join(merge)
			pyjs = python_to_pythonjs( '\n'.join(merge), cpp=True, module_path=module_path)
			...
			if cpyembed:  ## TODO move to per-mod option
				inlinepy = ('\n'.join(cpyembed)).replace('\n', '\\n').replace('"', '\\"')
				staticstr = 'const char* __python_main_script__ = "%s";\n' %inlinepy
				cppcode = staticstr + cppcode

			## gets compiled below
			#modules['c++'].append({'code':cppcode, 'index':n+1, 'links':cpp_links, 'include-dirs':cpp_idirs, 'defines':cpp_defines})
			modules['c++'].append({
	...
	if modules['c']:
		...
		if source:
			...
			if dynamiclib:
				...
				## when a static library is linked into a shared library compiled with position independent code (PIC)
				## the static library also needs to be compiled with `-fPIC`
				cmd.extend(['-fPIC', '-o', object_path ])
...
def main():
	...
	if singleout:
		...
		if not len(package['executeables']) and not package['html'] and not package['javascript']:
			print 'WARNING: no executeables or scripts in package'
			if package['datafiles']:
				for k in package['datafiles']:
					print 'output file: ' + k
			else:
				for k in package:
					print(k)
